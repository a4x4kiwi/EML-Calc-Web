...
class TreeRenderer {
  ...
  _bindEvents() {
    ...
    c.addEventListener('pointerup', e => {
      ...
      this._lastPointerUp = { t: now, x: e.offsetX, y: e.offsetY };
      // Detect double-tap: two taps within 350ms at roughly the same spot
      if (last && now - last.t < 350 &&
          Math.abs(e.offsetX - last.x) < 20 && Math.abs(e.offsetY - last.y) < 20) {
        this._lastPointerUp = null; // consume both taps
        this._handleExpand(e.offsetX, e.offsetY);
        return;
      }
      // Delay single-click to allow double-tap detection to win
      const ox = e.offsetX, oy = e.offsetY;
      setTimeout(() => {
        if (this._lastPointerUp && this._lastPointerUp.t === now) {
          this._handleClick(ox, oy);
        }
      }, 360);
    });
    ...
    c.addEventListener('contextmenu', e => {
      e.preventDefault();
      this._lastPointerUp = null; // cancel pending single-click
      this._handleExpand(e.offsetX, e.offsetY);
    });

    // dblclick fires after two clicks have already been processed by pointerup,
    // so we handle double-tap there and ignore the dblclick event entirely.
